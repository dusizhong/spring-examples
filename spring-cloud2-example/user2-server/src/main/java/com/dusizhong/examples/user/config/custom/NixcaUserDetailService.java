...
@Slf4j
@Service
public class NixcaUserDetailService {
    ...
    public UserDetails loadUserByCertAndRandomAndSign(String cert, String random, String sign) {
        ...
        if(!NixcaSvsInit.verifySign(cert, random, sign)) throw new UsernameNotFoundException("证书验签失败");
        if(StringUtils.isEmpty(NixcaSvsInit.getSerialNumber(cert))) throw new UsernameNotFoundException("解析证书序列号失败");
        SysUserCa sysUserCa = sysUserCaRepository.findByCaTypeAndSerialNumber(CaEnum.NIXCA.getCode(), NixcaSvsInit.getSerialNumber(cert));
        if(ObjectUtils.isEmpty(sysUserCa)) throw new UsernameNotFoundException("证书未绑定或已过期，请重新绑定");
